...
class Encoder(nn.Module):
    """
    Encoder for VQ-VAE when input‐frames are 256×240 RGB pairs.
    - Input: Concatenated (frame_t, frame_t+1) as (B, 6, 240, 256) 
    - Output: Latent feature map (B, 128, 7, 8)
    
    We downsample 5× with Conv2d(k=4, s=2, p=1):
      240×256 → 120×128 → 60×64 → 30×32 → 15×16 → 7×8
    """
    def __init__(self, in_channels=6, hidden_dims=[64, 128, 256, 512, 512], out_dim=128):
        super().__init__()
        layers = []
        ...
        for c_out in hidden_dims:
            # Always do Conv2d(kernel=4, stride=2, pad=1)
            layers.append(nn.Conv2d(c_in, c_out, kernel_size=4, stride=2, padding=1))
            layers.append(nn.BatchNorm2d(c_out))
            layers.append(nn.ReLU(inplace=True))
            c_in = c_out
        ...
        self.conv = nn.Sequential(*layers)
        # Project final 512→128 so we get (B, 128, 7, 8)
        self.project = nn.Conv2d(hidden_dims[-1], out_dim, kernel_size=1)

    def forward(self, x):
        # x = (B, 6, 240, 256)
        x = self.conv(x)         # → (B, 512, 7, 8)
        x = self.project(x)      # → (B, 128, 7, 8)
        return x                 # (B, 128, 7, 8)


class VectorQuantizer(nn.Module):
    """
    (unchanged from before, except that z is now (B, 128, 7, 8) instead of (B, 128, 5, 7))
    """
    def __init__(self, num_embeddings=256, embedding_dim=128, commitment_cost=0.25):
        super().__init__()
        self.embedding_dim = embedding_dim
    ...
    def forward(self, z):
        ...
        quantized = self.embeddings(encoding_indices)  # (B*H*W, 128)
        quantized = quantized.view(B, H, W, self.embedding_dim).permute(0, 3, 1, 2).contiguous()
        # Now quantized = (B, 128, 7, 8)
...
class Decoder(nn.Module):
    """
    Decoder for VQ-VAE when reconstructing a 256×240 frame.
    - Input: quantized latent (B, 128, 7, 8) + current frame (B, 3, 240, 256)
    - Output: reconstructed next frame (B, 3, 256, 240)
    """
    def __init__(self, 
                 in_channels=128,         # latent‐dim
                 cond_channels=3, 
        ...
        # Combine latent (128) + cond_feat (128) → 512
        self.fc = nn.Conv2d(in_channels + 128, hidden_dims[0], kernel_size=1)

        up_layers = []
        c_in = hidden_dims[0]  # 512
        for c_out in hidden_dims[1:]:
            # Upsample by 2 using ConvTranspose2d(k=4, s=2, p=1)
            up_layers.append(nn.ConvTranspose2d(c_in, c_out, kernel_size=4, stride=2, padding=1))
            up_layers.append(nn.BatchNorm2d(c_out))
            up_layers.append(nn.ReLU(inplace=True))
            c_in = c_out
        # Final upsample to get “close” to (240, 256). We’ll interpolate to exactly (256, 240) at the end.
        up_layers.append(nn.ConvTranspose2d(c_in, out_channels, kernel_size=4, stride=2, padding=1))
    ...
    def forward(self, z, cond):
        """
        z:    (B, 128, 7, 8)
        cond: (B,   3, 240, 256)  # current frame
        """
        # 1. Extract cond_feat at “bigger” resolution
        cond_feat = self.cond_conv(cond)  
        #    → cond_feat is (B, 128, 240, 256)

        # 2. Shrink cond_feat to match latent’s spatial (7, 8):
        cond_feat = F.interpolate(cond_feat, size=z.shape[2:], mode='bilinear', align_corners=False)
        #    → cond_feat is now (B, 128, 7, 8)

        # 3. Concatenate and push through “fc” to get initial hidden (512, 7, 8)
        x = torch.cat([z, cond_feat], dim=1)  # (B, 128+128, 7, 8)
        x = self.fc(x)                       # (B, 512, 7, 8)

        # 4. Upsample step‐by‐step:
        x = self.up(x)
        # After upconv1:  (B, 512, 15, 16)
        # After upconv2:  (B, 512, 30, 32)
        # After upconv3:  (B, 256, 60, 64)
        # After upconv4:  (B, 128, 120, 128)
        # After upconv5:  (B,  64, 240, 256)
        # After “last” upconv: (B, 3, 480, 512)?  Actually with 5 ups you’ll go 7→15→30→60→120→240→  (we’ve only appended five ConvTransposes above, so final is (B, 3, 240*2=480, 256*2=512)… which overshoots.)
        #
        #   (To avoid that, we typically only use 5 ups if starting from (7,8):
        #    7→15→30→60→120→240  (that’s exactly five halves: 7→15 (1), 15→30 (2), 30→60 (3), 60→120 (4), 120→240 (5).)
        #    BUT 8→16→32→64→128→256 exactly five ups. So that gets (240×256). Check carefully that your `self.up`
        #    has the correct number of ConvTranspose2d stages. The code above was “hidden_dims=[512,512,256,128,64]”
        #    which creates 5 ConvTranspose2d layers, ending with 64→3. That exactly goes:
        #       (7,8) → (15,16) → (30,32) → (60,64) → (120,128) → (240,256).
        #    So **you do not overshoot**—you land squarely at (240,256). No extra “(480,512)” stage.)
        #
        # 5. Finally, force‐resize to (256, 240):
        x = F.interpolate(x, size=(256, 240), mode='bilinear', align_corners=False)
        return x
